...
function camPlayerMatchesFilter(player, filter)
{
	switch(filter) {
		case ALL_PLAYERS:
			return true;
		case ALLIES:
			return player === CAM_HUMAN_PLAYER || allianceExistsBetween(player, CAM_HUMAN_PLAYER);
		case ENEMIES:
			return player >= 0 && player < CAM_MAX_PLAYERS && player !== CAM_HUMAN_PLAYER && !allianceExistsBetween(player, CAM_HUMAN_PLAYER);
		default:
